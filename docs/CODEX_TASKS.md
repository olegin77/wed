...
- [x] T-0237 | Enquiry create/update: API для пары (USER) и вендора (VENDOR)
...
- [x] T-0238 | Инвойсы: модель и генерация PDF-заглушки (HTML → PDF)
  - depends: [T-0226]
  - apply:
    ```bash
    set -euo pipefail
...
- [x] T-0239 | PDF генерация (puppeteer-free stub: сохраняем HTML)
  - depends: [T-0238]
  - apply:
    ```bash
    set -euo pipefail
...
- [x] T-0240 | S3 клиент (DigitalOcean Spaces): загрузка/получение URL
  - depends: [T-0003]
  - apply:
    ```bash
    set -euo pipefail
...
- [x] T-0241 | API загрузки медиа вендора (обложка площадки)
...
- [x] T-0242 | Redis-stub (in-memory) и простой кэш-хелпер
  - depends: []
  - apply:
    ```bash
    set -euo pipefail
...
- [x] T-0244 | Очереди BullMQ-stub (уже есть): таск для генерации превью
